...
def inference(args, multimask_output, db_config, model,folder_path, test_save_path=None,):
    ...
    return performance,mean_hd95,loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lora_ckpt', type=str, default='checkpoints/epoch_5_bd1_ld0.1_out1.pth', help='The checkpoint from LoRA')
    ...
    file_path = 'results_all.txt'

    file_path='Exp_log'+'/'+file_path[:-4]+'_layer'+str(args.sftm_hpyer)+'_stepsize'+str(args.att_step_size)+'_bound'+str(args.att_bound)+'_steps'+str(args.att_numstep)+ file_path[-4:]
    if not os.path.exists(file_path):
        os.makedirs(os.path.dirname(file_path), exist_ok=True)

    with open(file_path, 'a') as file:
        file.write(f"path:{args.noise_path}, model:{args.lora_ckpt}\n")
