...
for i_batch, sampled_batch in tqdm(enumerate(testloader)): #every-time one image from testloader
    ...
    for ind in range(image.shape[0]):
        ...
        if args.UMI_GR==1:
            ...
            GE = False    
            step=0
        perturbations = attacker(perturb_img, targets)
        perturb_img = Variable(perturb_img.data + perturbations, requires_grad=True)
        ...
        for _ in range(step):
            ...
            input_image_torch_t = input_image_torch_t 
