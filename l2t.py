...
class L2T(Attack):
    def __init__(self, model_name, epsilon=10/255, alpha=1.6/255, epoch=10, decay=1., targeted=False, random_start=False,
    ...
    def forward(self, data, label, **kwargs):
            ...
            """
            attacker = L2T(model_name = self.model, targeted = False, epoch = 10, num_scale = 3)
            if self.targeted:
                assert len(label) == 2
            ...
            for e in range(self.epoch):
                ...
                for i in range(self.num_scale):
                    ...
                    features,features_layer = self.model.image_encoder(data+delta)
                    # logits = self.get_logits(self.transform(data+delta, search=rw_search))
                    loss =criterion(features*0.9999, label)
                    losses.append(loss)
                    

                # Calculate the loss
                loss = torch.sum(torch.stack(losses))/self.num_scale
                print(loss.item())
