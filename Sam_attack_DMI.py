...
for i_batch, sampled_batch in tqdm(enumerate(testloader)): #every-time one image from testloader
    ...
    for ind in range(image.shape[0]):
        ...
        for _ in range(args.att_numstep):
            # if _==0:
                # momentum=common_perturbation/ torch.norm(common_perturbation, p=1)*1
            if args.consider_lora:
            ...
            random_int = random.randrange(-5, 5)
            rotation_transform = transforms.RandomRotation((random_int, random_int))
            input_image_torch_t=rotation_transform(input_image_torch)
            mask = torch.ones_like(input_image_torch_t)
            num_pixels_to_mask = int(mask.numel() * 0.01)
            # 随机选择要掩盖的像素索引
            masked_indices = torch.randperm(mask.numel())[:num_pixels_to_mask]
            # 将选中的像素点掩盖掉，即设置为 0或其他你希望的值
            mask.view(-1)[masked_indices] = 0
            input_image_torch_t = input_image_torch_t * mask
            with torch.no_grad():
                targets_rand,targets_layer_rand=sam.image_encoder(input_image_torch_t)    
            ...
            perturb_img_input_t = rotation_transform(perturb_img_input)
            perturb_img_input_t = perturb_img_input_t * mask
            ...
            if _==-1:
                ...
                loss_r=calculate_loss(features_r,targets_rand,features_layer_r,targets_layer_rand,GE)
                loss = loss+0.25*loss_r
                ...
                # upd_dir  = 0.5 * momentum.detach() +grad / torch.norm(grad, p=1)+smoothed_direction*0.5
                # smoothed_direction=smoothed_direction*0.5
