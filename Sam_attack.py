    ...
    for ind in range(image.shape[0]):
        ...
        for _ in range(args.att_numstep):
            ...
            if mask_torch.sum() >-1:
                ...
                del features,features_layer
                grad= torch.autograd.grad(loss, perturb_img,retain_graph=True, create_graph=False)[0]
                grad=grad / torch.norm(grad, p=1)

                mean_abs = torch.mean(torch.abs(grad))
                mask_grad = torch.abs(grad)>mean_abs
                # grad=grad*mask_grad
                momentum = 0.5 * momentum.detach()+1 *grad
        ...
        adv_noise= eta.clone().detach().cpu()
        save_image(adv_noise[:,0:1,:,:]*5, 'output_image.png')
        adv_noise = adv_noise.numpy()
        adv_noise = (adv_noise*255).astype('uint8')
        # print(adv_noise.max(),adv_noise.min())
        noise_image = Image.fromarray(adv_noise[0,0,:,:], mode='L')
        noise_image.save('output_image_grey.png')
